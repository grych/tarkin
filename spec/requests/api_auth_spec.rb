...
describe "API Authorization" do
  ...
  it "get a token via GET" do
    get '/_api/v1/authorize?email=email0@example.com&password=password0'
    expect(response).to be_success
    expect(response.body.length).to be > 10
  end
  it "not get a token via GET" do
    get '/_api/v1/authorize?email=email0@example.com&password=password0x'
    expect(response).not_to be_success
    expect(response.body).to eq "Unathorized"
  end
  it "get a token via POST" do
    post '/_api/v1/authorize', email: "email0@example.com", password: "password0"
    expect(response).to be_success
    expect(response.body.length).to be > 10
  end
  it "not get a token via POST" do
    post '/_api/v1/authorize', email: "email0@example.com", password: "password0x"
    expect(response).not_to be_success
    expect(response.body).to eq "Unathorized"
  end
end
